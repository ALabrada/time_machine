...
class PicturePageState extends State<PicturePage>
  ...
  Widget _buildContent({Picture? picture}) {
    return FullScreenView(
      collapsible: true,
      animationController: animationController,
      topBar: _buildAppBar(picture: picture),
      bottomBar: _buildToolbar(picture: picture),
      content: picture == null
          ? null
          : PhotoView(
              imageProvider: PictureFrame.imageFor(
                picture.url,
                databaseService: context.read(),
              ),
              scaleStateChangedCallback: (state) async {
                ...
                } else if (animationController.value == 1.0 &&
  ...
  PreferredSizeWidget _buildToolbar({Picture? picture}) {
